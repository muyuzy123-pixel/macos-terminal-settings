...
The current READMEs now use separate English and Simplified Chinese screenshots, captured directly from the isolated app window as native 2120×1520 RGBA PNGs (1060×760 points at 2×). They retain transparent rounded corners and the native window frame, omit the Computer Use pointer and capture shadow, and avoid JPEG conversion or enlargement. Current values 35/37 and unapplied drafts 48/64 are visible in both languages. A new before/after audit passed with all 46 production addresses, power settings, app state, recovery files, and residue inventory unchanged.

The original 2026-09-20 image and publication JSON are retained as historical evidence; they no longer supply the current README illustrations. The correction is documented in [screenshots-2026-09-22.json](publication/screenshots-2026-09-22.json).
